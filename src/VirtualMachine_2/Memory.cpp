...
Memory::Memory(size_t a_data_memory_size, size_t a_orders_memory_size, std::vector<int64_t> a_orders_memory)
:m_data_memory_size(a_data_memory_size)
, m_orders_memory_size(a_orders_memory_size)
...
int64_t Memory::get_Instruction(size_t a_orders_index) const
{
...
void Memory::set_orders_to_memory(std::vector<int64_t> a_instructions)
{
    m_orders_memory = a_instructions;
}


void Memory::print_orders()
{
    auto begin = m_orders_memory.begin();
    auto end = m_orders_memory.end();
